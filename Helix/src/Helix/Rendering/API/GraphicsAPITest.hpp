...
namespace hlx::gfx
{
    ...
    std::shared_ptr<VertexBuffer>           create_vertex_buffer(size_t size)
    {
    ...
    template<typename T>
    std::shared_ptr<UniformArrayBuffer<T>>  create_uniform_array_buffer(unsigned int binding, unsigned int count)
    {
        return std::make_shared<OpenGLUniformArrayBuffer<T>>(binding, count);
    }
    template<typename T>
    std::shared_ptr<UniformArrayBuffer<T>>  create_uniform_array_buffer(unsigned int binding, std::span<const T> data)
    {

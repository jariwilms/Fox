...
export namespace fox::random
{
    namespace distribution
    {
        ...
        auto normal   (fox::interval_t<T> interval = fox::interval_t<T>{})
        {
            return std::normal_distribution<T>{ interval.min, interval.max }(::engine);
        }
        template<typename T> requires (std::is_arithmetic_v<T>)
        auto uniform  (fox::interval_t<T> interval = fox::interval_t<T>{})
        {
        ...
        auto binomial (fox::count_t trials = fox::count_t{ 1u }, fox::float64_t probability = fox::float64_t{ 0.5 })
        {
            return std::binomial_distribution<T>{ trials, probability }(::engine);
        } 
        template<typename T> requires (std::is_integral_v<T> and std::is_unsigned_v<T>)
        auto bernoulli(fox::float64_t probability = fox::float64_t{ 0.5 })
        {
            return std::bernoulli_distribution{ probability }(::engine);
        }
        template<typename T> requires (std::is_integral_v<T> and std::is_unsigned_v<T>)
        auto poisson  (fox::float64_t mean = fox::float64_t{ 1.0 })
        {
    ...
    template<typename T>
    auto next   (fox::interval_t<T> interval = fox::interval_t<T>{})
    {
